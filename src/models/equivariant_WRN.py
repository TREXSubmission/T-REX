"""
modified wide resnet implementation from https://github.com/QUVA-Lab/escnn/blob/master/examples/e2wrn.py
which is in turn adapted from implementation of wide resnet at
https://github.com/meliketoy/wide-resnet.pytorch/blob/master/networks/wide_resnet.py available in src.models.wide_resnet
"""
...
from src.models.wide_resnet import wide_basic, Wide_resnet
...
class WideBasic(enn.EquivariantModule):
    ...
    def __init__(self,
        ...
        self.dropout = enn.PointwiseDropout(inner_type, p=self.dropout_rate)

        self.conv2 = conv(inner_type, self.out_type, stride=self.stride)

        self.shortcut = None
        if self.stride != 1 or self.in_type != self.out_type:
            self.shortcut = conv1x1(self.in_type, self.out_type, stride=self.stride, bias=False)
    ...
    def export(self):
        """
        export each layer to pytorch and combine them into src.models.wide_resnet.wide_basic
        pytorch block of corresponding sizes, and set it to eval mode
        :return: pytorch wide_basic block from src.models.wide_resnet
        """
        self.eval()

        exported = wide_basic(self.in_type.size,
                              self.inner_type.size,
                              self.dropout_rate,
                              self.stride)
        exported.bn1 = self.bn1.export()
        exported.relu1 = self.relu1.export()
        exported.conv1 = self.conv1.export()

        exported.bn2 = self.bn2.export()
        exported.relu2 = self.relu2.export()

        exported.dropout = self.dropout.export()
        exported.conv2 = self.conv2.export()

        if self.shortcut:
            exported.shortcut = self.shortcut.export()
        exported.eval()
        return exported
...
class Wide_ResNet(torch.nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes=100,
        ...
        self.bn1 = enn.InnerBatchNorm(self.layer3.out_type, momentum=0.9)
        self.relu = enn.ReLU(self.bn1.out_type, inplace=True)
        self.linear = torch.nn.Linear(self.bn1.out_type.size, num_classes)
    ...
    def export(self):
        """
        export each layer of the network to corresponding pytorch layers or
        src.models.wide_resnet.wide_basic blocks and combine those layers into
        src.models.wide_resnet.Wide_resnet pytorch model with same dimensions,
        and set to eval mode
        :return: pytorch Wide_resnet model from src.models.wide_resnet
        """
        exported = Wide_resnet(self.depth,
                               self.widen_factor,
                               self.dropout_rate,
                               self.num_classes)

        exported.conv1 = self.conv1.export()
        exported.layer1 = self.layer1.export()
        exported.layer2 = self.layer2.export()
        exported.layer3 = self.layer3.export()

        exported.bn1 = self.bn1.export()
        exported.relu = self.relu.export()

        exported.linear = self.linear
        exported.eval()
        return exported
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rot90', action='store_true', default=False,
                        help='Makes the model invariant to rotations of 90 degrees')
